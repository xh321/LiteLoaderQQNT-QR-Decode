import { decodeQRUrl, encodeQR } from "./decodeQR.js";
 ...
 */
function addQContextMenu(qContextMenu, id, icon, title, callback) {
...
function addMainQContextMenu() {
  ...
  document.addEventListener("mouseup", (event) => {
    isCkEditor =
      event.target.classList.contains("ck") ||
      event.target.classList.contains("ck-placeholder") ||
      event.target.dataset.placeholder != null;
    if (event.button === 2) {
      imgEl = event.target;
      if (
  ...
  new MutationObserver(() => {
    ...
    if (qContextMenu && !imagePath && isCkEditor) {
      addQContextMenu(
        qContextMenu,
        "qr-encode-menu",
        `
        ...
        async () => {
          ...
          try {
            var encoded = await encodeQR(text);
            await window.qr_decode.previewQrImg(encoded);
          } catch (e) {
            console.log("[QR-Decode]", "编码失败", e);
